...
const createServicePackageJson = (packageJson) => ({
  ...
  scripts: {
    ...
    'd1:migrate': 'node d1-migrate.mjs',
  },
...
const createStartScript = () => [
  ...
  '',
  '  // 先执行 D1 数据库迁移，确保表结构已就绪。',
  "  console.info('[start-production] 正在检查 D1 数据库迁移')",
  "  const migrateResult = await runCommand('node', ['d1-migrate.mjs'])",
  "  const migrationSummary = summarizeD1MigrateOutput(`${migrateResult.stdout}\\n${migrateResult.stderr}`)",
  "  console.info(",
...
const copyRuntimeFiles = async () => {
  ...
  // 复制 D1 迁移脚本与 SQL 迁移文件，供容器启动时执行。
  const d1MigrateScriptPath = path.resolve(rootDir, 'scripts', 'd1-migrate.mjs')
  try {
    await fs.cp(d1MigrateScriptPath, path.resolve(outputDir, 'd1-migrate.mjs'))
  } catch (error) {
    if (error && error.code !== 'ENOENT') {
      throw error
    }
  ...
  const migrationsSourceDir = path.resolve(rootDir, 'migrations')
  try {
    await fs.access(migrationsSourceDir)
    await fs.cp(migrationsSourceDir, path.resolve(outputDir, 'migrations'), { recursive: true })
  } catch (error) {
    if (error && error.code !== 'ENOENT') {
      throw error
    }
